...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    for ($i = 1; $i <= $jumlah_tiket; $i++) {
        ...
        $jalan = mysqli_query($konek, "insert into booking (id_booking, id_user, tanggal_pesan, tanggal_booking, kode_booking , harga, status,id_harga,jam_pesan) values ('$id_book', '$iduser', '$tanggal_pesan','$tanggal_kunjungan', '$randomnomor','$harga' ,'$status','$kode_tiket','$jam_pesan')");
        header("location: pembayaran.php?id_harga=$id_harga&jumlah_tiket=$jumlah_tiket&harga=$harga&total=$total&id_user=$iduser&id_booking=$id_book");
    }
}
